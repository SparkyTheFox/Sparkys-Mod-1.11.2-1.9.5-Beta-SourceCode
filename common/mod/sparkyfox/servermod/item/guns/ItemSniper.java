...
import net.minecraftforge.fml.relauncher.SideOnly;
 ...
 public class ItemSniper extends ItemBasic
 {
   public ItemSniper()
   {
     setMaxDamage(1000);
...
///////////////////////////////////////////     
     addPropertyOverride(new ResourceLocation("pull"), new IItemPropertyGetter() {
       ...
       public float apply(ItemStack stack, World worldIn, EntityLivingBase entityIn) {
         ...
         return (itemstack != null) && (itemstack.getItem() == ItemSniper.this) ? (stack.getMaxItemUseDuration() - entityIn.getItemInUseCount()) / 20.0F : 0.0F;
       }
     });
...
///////////////////////////////////////////
   ...
   public void onPlayerStoppedUsing(ItemStack stack, World worldObj, EntityLivingBase entity, int count)
   {
     ...
     if ((entity instanceof EntityPlayer)) {
       ...
       if (!player.capabilities.isCreativeMode)//Translation: if the player is not in creative mode 
         consumeItem(player, ModItems.SniperRounds);//Translation: then the Sniper WILL use Ammo
       compound.setBoolean("IsLoaded2", false);//Translation: then the sniper will not be loaded after its used ammo
     }
     
     stack.damageItem(1, entity);//Translation: Every shot fire will cause 1 damage point to the sniper
     ...
     projectile.damage = 20.0F;//Translation: set the damage the porjectile will do)
     projectile.setSpeed(40);//Translation: set the velocity of the profectile(value of speed)
     projectile.shoot(0.1F);//Translation: will shoot the projectile(the float is the seeed accuracy-lower the better)
     worldObj.spawnEntity(projectile);//Translation: Spawn the set projectile which is this -----^^^(ModWeapons.bullet)
     
     playSound(entity, ModSoundEvents.sniper, 1.0F, 1.0F);
 ...
 ///////////////////////////////////////////
   ...
   public void onUsingTick(ItemStack stack, EntityLivingBase entity, int count)
   {
     ...
     if ((!player.capabilities.isCreativeMode) && (stack.hasTagCompound()) && 
       (stack.getTagCompound().getBoolean("Reloading2")) && (hasItem(player, ModItems.SniperRounds)) && 
       (ticks == 2)) {//how long it takes to reload
       playSound(player, ModSoundEvents.sniperreload, 1.0F, 1.0F);
 ...
 ///////////////////////////////////////////
   ...
   public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand)
   {
     ...
     if ((!player.capabilities.isCreativeMode) && (hasItem(player, ModItems.SniperRounds)) && (!compound.getBoolean("IsLoaded2")))
